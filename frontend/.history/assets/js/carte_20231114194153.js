...
export function displayMenuItems(menuData, category) {
  ...
  menuData
    ...
    .forEach((item) => {
      ...
      // 材料（ingredients）和 after 部分
      const ingredientsAfterDiv = document.createElement('div')
      ingredientsAfterDiv.className = 'ingredients-after'

      let contentAdded = false // 用于跟踪是否已添加内容

      // 材料（ingredients）
      if (item.ingredients) {
        const ingredientsSpan = document.createElement('span')
        ingredientsSpan.textContent = item.ingredients.join(', ')
        ingredientsSpan.className = 'menu-item-ingredients'
        ingredientsAfterDiv.appendChild(ingredientsSpan)
        contentAdded = true
      }
      ...
      if (item.after && item.after.length > 0) {
        // 如果之前已添加材料，先添加逗号和空格
        if (contentAdded) {
          ingredientsAfterDiv.appendChild(document.createTextNode(', '))
        }

        const afterSpan = document.createElement('span')
        afterSpan.textContent = item.after.join(', ')
        afterSpan.className = 'menu-item-after hithlight'
        ingredientsAfterDiv.appendChild(afterSpan)
      }

      textContainer.appendChild(ingredientsAfterDiv)
