...
// 读取 JSON 文件
let jsonFile = 'menu.json' // 替换为您的 JSON 文件路径
let jsonData = JSON.parse(fs.readFileSync(jsonFile, 'utf8'))

// 图片目录
let imageDir = './img' // 替换为您的图片目录路径

// 网络图片根目录
let imageRootURL = 'http://15.188.50.31/backend/img/' // 替换为网络图片根目录
...
walkDir(imageDir, function (filePath) {
  ...
  let relativePath = path.relative(imageDir, filePath).replace(/\\/g, '/')
  imagePaths[imageName] = imageRootURL + relativePath
})
